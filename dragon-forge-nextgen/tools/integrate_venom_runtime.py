#!/usr/bin/env python3
"""Apply the reviewed Nox runtime integration exactly once.
This script is a temporary development integrator: it edits only named UTF-8 source
files and refuses ambiguous/missing anchors. Generated GLB/PNG assets are built by
the separate offline authoring script and independently validated before commit.
"""
...
def rep(rel: str, old: str, new: str) -> None:
    ...
    if count != 1:
        raise RuntimeError(f"{rel}: expected exactly one anchor, found {count}: {old[:100]!r}")
    p.write_text(text.replace(old, new, 1))

# Imported guardian controller.
rep('campaign/dragon.gd',
    'const StoneRig = preload("res://campaign/stone_rig.gd")\n',
    'const StoneRig = preload("res://campaign/stone_rig.gd")\nconst VenomRig = preload("res://campaign/venom_rig.gd")\n')
rep('campaign/dragon.gd','if not id in ["fire","ice","storm","stone"]:',
    'if not id in ["fire","ice","storm","stone","venom"]:')
rep('campaign/dragon.gd',
    'if id == "stone":\n\t\t\tnext = StoneRig.new()\n\t\telif id == "storm":',
    'if id == "venom":\n\t\t\tnext = VenomRig.new()\n\t\telif id == "stone":\n\t\t\tnext = StoneRig.new()\n\t\telif id == "storm":')

rep('campaign/growth.gd',
    'static func form_name(guardian: String, evolved: bool) -> String:\n\tif guardian == "stone": return "Cairn"',
    'static func form_name(guardian: String, evolved: bool) -> String:\n\tif guardian == "venom": return "Nox"\n\tif guardian == "stone": return "Cairn"')
rep('campaign/audio/synth.gd',
    '{"fire":.78,"ice":1.55,"storm":1.08,"stone":.58}.get(guardian,1.0)',
    '{"fire":.78,"ice":1.55,"storm":1.08,"stone":.58,"venom":.90}.get(guardian,1.0)')

# Separate Frozen Vault culture plinth; the Ice egg remains untouched.
rep('campaign/room.gd','var stone_imprint: Node3D\n','var stone_imprint: Node3D\nvar venom_culture: Node3D\n')
rep('campaign/room.gd',
...
# Target-owned Toxin; shields block new application, not already-applied ticks.
rep('campaign/enemy.gd','const Roles = preload("res://campaign/enemy_roles.gd")\n',
    'const Roles = preload("res://campaign/enemy_roles.gd")\nconst GuardianCombat = preload("res://campaign/guardian_combat.gd")\n')
rep('campaign/enemy.gd','var charged = 0.0\nvar impact_flash = 0.0\n',
    'var charged = 0.0\nvar toxin = 0\nvar toxin_time = 0.0\nvar toxin_tick = 1.0\nvar impact_flash = 0.0\n')
rep('campaign/enemy.gd',
...
# HUD: fifth roster, Toxin readout and explicit canonical recipe.
rep('campaign/hud.gd','STONE RESONANCE   /   22 ROOMS   /   FOUR GUARDIANS, TWO FIELD SLOTS',
    'VENOM RESONANCE   /   22 ROOMS   /   FIVE GUARDIANS, TWO FIELD SLOTS')
rep('campaign/hud.gd',
...
rep('campaign/hud.gd','\tif foe.charged>0.0:\n\t\tenemy_readout.text+="  /  CHARGED %.1fs" % foe.charged\n',
    '\tif foe.charged>0.0:\n\t\tenemy_readout.text+="  /  CHARGED %.1fs" % foe.charged\n\tif foe.toxin>0:\n\t\tenemy_readout.text+="  /  TOXIN x%d %.1fs" % [foe.toxin,foe.toxin_time]\n')
rep('campaign/hud.gd','\tfor id in ["fire","ice","storm","stone"]:\n','\tfor id in ["fire","ice","storm","stone","venom"]:\n')
rep('campaign/hud.gd','{"fire":GOLD,"ice":TEAL,"storm":Color("c4b1fa"),"stone":Color("c8ad7c")}[id]',
    '{"fire":GOLD,"ice":TEAL,"storm":Color("c4b1fa"),"stone":Color("c8ad7c"),"venom":Color("a8db61")}[id]')
rep('campaign/hud.gd',
...
rep('campaign/hud.gd','\t_wrapped(overlay_column,"The Forge now holds two explicit, non-destructive resonance recipes. No random failure, parent sacrifice or salvage cost.",18,PAPER)\n',
...
# Inspector.
rep('validation/character_inspection.gd','"tempest_arc", "stone_guardian", "buffer_overflow"',
    '"tempest_arc", "stone_guardian", "venom_guardian", "buffer_overflow"')
rep('validation/character_inspection.gd',
...
print('VENOM_RUNTIME_INTEGRATION: source anchors applied')
