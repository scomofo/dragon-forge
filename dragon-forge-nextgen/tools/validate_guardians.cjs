...
const folder=path.resolve(__dirname,'../campaign/guardians');
(async()=>{ let errors=0,warnings=0; const reports={};
  for(const file of ['ice_guardian.glb','ice_egg.glb']) {
  ...
  fs.mkdirSync(path.resolve(__dirname,'../artifacts/guardians'),{recursive:true});
  fs.writeFileSync(path.resolve(__dirname,'../artifacts/guardians/gltf-validation.json'),JSON.stringify(reports,null,2));
  console.log(`GUARDIAN_GLTF: 2 assets, ${errors} errors, ${warnings} warnings`);
