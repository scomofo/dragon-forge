#!/usr/bin/env python3
"""Validate committed Nox/Venom exports without rebuilding them."""
import hashlib,json,struct
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1];FOLDER=ROOT/'campaign'/'venom';checks=0
def check(ok,msg):
...
m=json.loads((FOLDER/'manifest.json').read_text());check(m.get('source')=='tools/build_venom_guardian.py','editable source recorded')
...
print(f'VENOM_EXPORTS: {checks} checks, 0 failures / {vertices} vertices / {triangles} triangles / {joints} joints')
