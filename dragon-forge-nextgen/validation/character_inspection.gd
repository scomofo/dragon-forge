...
const ACTORS = ["magma_guardian", "firewall_sentinel", "packet_warden", "ice_guardian", "magma_evolved", "rime_evolved", "storm_guardian", "tempest_arc", "stone_guardian", "venom_guardian", "shadow_guardian", "void_guardian", "light_guardian", "synthesis_guardian", "buffer_overflow", "memory_leak", "stack_overflow", "mirror_admin", "singularity"]
...
func _build_controls() -> void:
	...
	UI.picker(box, ["Magma guardian", "Firewall Sentinel", "Packet Warden", "Rime / Ice guardian", "Crowned Magma / Evolved", "Aurora Rime / Evolved", "Arc / Storm fusion", "Tempest Arc / Evolved", "Cairn / Stone guardian", "Nox / Venom guardian", "Umbra / Shadow guardian", "Null / Void guardian", "Lumen / Light guardian", "Prism / Synthesis guardian", "Buffer Overflow / Boss", "Memory Leak / Boss", "Stack Overflow / Boss", "Mirror Admin / Boss", "Singularity / Final"], load_actor)
...
func load_actor(index: int) -> void:
	...
	elif actor_id == "synthesis_guardian":
		model = load("res://campaign/synthesis/synthesis_guardian.glb").instantiate();add_child(model)
...
func set_view(index: int) -> void:
	...
	if actor_id in ["void_guardian","synthesis_guardian"]:
		focus=[Vector3(0,1.4,0),Vector3(0,1.8,0),Vector3(0,.6,0),Vector3(0,1.4,-.8),Vector3(0,1.4,.6)][index]
