...
**Open `project.godot` in Godot 4.6.3 Standard and press F5.** Run Project launches the complete Reconnection campaign: **22 connected rooms, four sectors, 13 encounters and an ending**. The current development pass adds Lumen, the Light guardian, and campaign save schema 10. Release acceptance for this revision must be established by its own workflow results.
...
Up to **eight guardians** can be owned: Magma, Rime, Arc, Cairn, Nox, Umbra, Null and Lumen. Expeditions have **two field slots**. Select the active guardian and reserve at the Forge Nursery. Optional recruitment can happen in any order allowed by each guardian's actual prerequisites; recruiting another guardian preserves an existing expedition pair and keeps the campaign saveable.
...
Valid older campaign saves migrate in memory to schema 10; loading preserves their original bytes and the first later successful write retains a backup. Completed older campaigns receive the earned Light guardian during migration; unfinished campaigns do not. Optional ownership order and the selected two-guardian pair survive migration. Unknown, corrupt or unearned states remain protected from overwrite.
