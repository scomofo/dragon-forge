...
const STRIDE = 1.90 # metres per complete L/R cycle
const DUTY = 0.50
const LIFT = 0.16
	...
	for side in ["L", "R"]:
		...
		for p in candidates:
			if p.y <= low + 0.025:
				center += p
				count += 1
		assert(count > 0, "Magma foot contact requires weighted sole geometry")
		center /= count
...
func apply(delta: float, state: Dictionary) -> void:
	...
	for side in ["L", "R"]:
		...
		if leg.planted and Vector2(nominal.x-leg.anchor.x, nominal.z-leg.anchor.z).length() > 0.76:
			leg.planted = false
			leg.blocked = true
			leg.from = leg.last
			leg.settle = 0.0
		...
		var error = _solve(leg, target, leg.basis, -1 if side == "L" else 1)
