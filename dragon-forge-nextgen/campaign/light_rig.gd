extends Node3D
## Lumen is an imported gold/white glass biped; authored glide has no foot solver. Sampling never resolves combat.
const Scene = preload("res://campaign/light/light_guardian.glb")
...
func _ready() -> void:
	...
	assert(emitter_bone >= 0, "Lumen requires an authored head emitter")
