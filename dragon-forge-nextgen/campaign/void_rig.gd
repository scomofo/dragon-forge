extends Node3D
## Null is an imported hollow crystal frame. Sampling never resolves combat.
const Scene = preload("res://campaign/void/void_guardian.glb")
...
func _ready() -> void:
	...
	assert(emitter_bone >= 0, "Null requires an authored forward-rim emitter")
...
func animate(delta: float, speed: float, reduced: bool, state: Dictionary) -> void:
	...
	elif state.get("null_reflect", 0.0) > 0.0:
		sampled_clip = "guard"
		sampled_time = .6 if reduced else clampf(1.2 - state.null_reflect, 0.0, 1.2)
	elif state.guard:
		sampled_clip = "guard"
		sampled_time = .6
	elif hurt_time > 0.0:
