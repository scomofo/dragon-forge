extends Node3D
## Nox is a distinct imported frilled wyrm. Animation sampling is presentation-only.
const Scene = preload("res://campaign/venom/venom_guardian.glb")
...
func animate(delta:float,speed:float,reduced:bool,state:Dictionary)->void:
	...
	clock+=dt;hurt_time=maxf(0.0,hurt_time-dt);sampled_clip="idle";sampled_time=0.0 if reduced else fposmod(clock,1.7)
	if state.hp<=0.0:death_time=minf(.85,death_time+dt);sampled_clip="defeat";sampled_time=death_time
	elif state.action!="":sampled_clip=state.action;sampled_time=state.action_time
	elif state.guard:sampled_clip="guard";sampled_time=.45
	elif hurt_time>0.0:sampled_clip="hurt";sampled_time=.24-hurt_time
	elif speed>.15:sampled_clip="walk";sampled_time=0.0 if reduced else fposmod(clock,.82)
	Art.sample(player,sampled_clip,sampled_time)
func muzzle_position()->Vector3:
	return skeleton.global_transform*(skeleton.get_bone_global_pose(skeleton.find_bone("Head"))*Vector3(0,-.04,-.62))
