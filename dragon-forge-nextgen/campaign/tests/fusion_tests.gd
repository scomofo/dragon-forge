...
func run() -> void:
	...
	for version in [1,2,3]:
		...
		check(migrated.version==11 and not migrated.storm_forged and not migrated.lattice_recovered and not migrated.shadow_forged,"old version %d reaches schema 11 with no unearned fusion or Shadow"%version)
	...
	check(actor_picker.item_count==19,"all nineteen actual actors appear in the inspection picker")
