...
func run() -> void:
	...
	for version in [1,2,3]:
		...
		check(migrated.version==9 and not migrated.storm_forged and not migrated.lattice_recovered and not migrated.shadow_forged,"old version %d reaches schema 9 with no unearned fusion or Shadow"%version)
	...
	check(actor_picker.item_count==17,"all seventeen actual actors appear in the inspection picker")
