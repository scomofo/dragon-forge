...
func eligible()->Dictionary:
	...
	return c
func run():
	var fresh=Rules.fresh();check(fresh.version==6,"fresh schema 6");check(not fresh.stone_imprint_recovered and not fresh.stone_forged,"fresh Stone flags false")
	var old=fresh.duplicate(true);old.version=5;old.erase("stone_imprint_recovered");old.erase("stone_forged")
	var migrated=Rules.normalize(old);check(migrated.version==6,"schema 5 migrates");check(not migrated.stone_imprint_recovered and not migrated.stone_forged,"migration grants no Stone progress")
