...
func eligible()->Dictionary:
	...
	return c
func run():
	var fresh=Rules.fresh();check(fresh.version==9,"fresh schema 9");check(not fresh.stone_imprint_recovered and not fresh.stone_forged,"fresh Stone flags false")
	...
	var migrated=Rules.normalize(old);check(migrated.version==9,"schema 5 migrates through Stone to schema 9");check(not migrated.stone_imprint_recovered and not migrated.stone_forged,"migration grants no Stone progress")
