...
func run() -> void:
	...
	for version in [1,2]:
		...
		check(migrated.version==5 and migrated.evolutions=={"fire":"","ice":"","storm":""}, "version %d migrates without auto-evolving" % version)
		...
		check(FileAccess.get_file_as_string(store.path)==bytes and loaded.version==5, "migration never writes on load")
