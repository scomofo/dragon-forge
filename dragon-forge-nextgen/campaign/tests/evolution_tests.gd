...
func run() -> void:
	...
	for version in [1,2]:
		...
		check(migrated.version==3 and migrated.evolutions=={"fire":"","ice":""}, "version %d migrates without auto-evolving" % version)
		...
		check(FileAccess.get_file_as_string(store.path)==bytes and loaded.version==3, "migration never writes on load")
