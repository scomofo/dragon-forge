...
func run() -> void:
	...
	for version in [1,2]:
		...
		check(migrated.version==11 and migrated.evolutions=={"fire":"","ice":"","storm":""}, "version %d migrates without auto-evolving through schema 11" % version)
		...
		check(FileAccess.get_file_as_string(store.path)==bytes and loaded.version==11, "migration reaches schema 11 without writing on load")
