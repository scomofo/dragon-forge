...
func _run() -> void:
	...
	for room_id in ["live-wire","logic-core"]:
		w.campaign=Fixtures.recruited();w.campaign.loadout=["fire","storm"];w.campaign.active_guardian="storm"
		...
		while is_instance_valid(w.enemy) and w.dragon.state.hp>0 and ticks<3600:
			...
			if open:
				...
				for id in (["burst","breath","claw"] if foe.charged>0.0 else ["breath","claw"]):
					if delta.length()<=Combat.rule(w.dragon.state,id).range and Combat.rejection(w.dragon.state,id)=="":
		...
		w.hud.close_overlay()
	DirAccess.make_dir_recursive_absolute(ProjectSettings.globalize_path("res://artifacts/fusion"))
	var f=FileAccess.open("res://artifacts/fusion/storm-controller-play.json",FileAccess.WRITE)
	...
	print("STORM_CONTROLLER_PLAY: %d failures"%failures)
