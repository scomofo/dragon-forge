...
func eligible()->Dictionary:
	...
	return c
func run():
	var fresh=Rules.fresh();check(fresh.version==7,"fresh schema 7");check(not fresh.venom_culture_recovered and not fresh.venom_forged,"fresh Venom flags false")
	var old=fresh.duplicate(true);old.version=6;old.erase("venom_culture_recovered");old.erase("venom_forged")
	var migrated=Rules.normalize(old);check(migrated.version==7,"schema 6 migrates");check(not migrated.venom_culture_recovered and not migrated.venom_forged,"migration grants no Venom progress")
