...
func run() -> void:
	...
	check(migrated.version==11 and migrated.guardians==["fire"],"version 1 migration initializes exactly Magma through schema 11")
	for key in legacy:
	...
	check(loaded.version==11,"disk migration readable at current schema")
