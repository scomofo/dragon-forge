...
func run() -> void:
	...
	check(migrated.version==Rules.SAVE_VERSION and migrated.guardians==["fire"],"version 1 migration initializes exactly Magma through current schema")
	for key in legacy:
	...
	check(loaded.version==Rules.SAVE_VERSION,"disk migration readable at current schema")
