...
func run() -> void:
	...
	check(migrated.version==10 and migrated.guardians==["fire"],"version 1 migration initializes exactly Magma through schema 10")
	for key in legacy:
	...
	check(loaded.version==10,"disk migration readable at current schema")
