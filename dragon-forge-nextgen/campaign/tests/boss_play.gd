...
func _run() -> void:
	...
	for room_id in ["overflow-vent","memory-vault","logic-core","protocol-throne","singularity"]:
		w.campaign=Rules.fresh();Rules.hatch(w.campaign)
		for z in Data.ZONES:
			if z.boss==room_id: break
		...
		w.hud.close_overlay()
	DirAccess.make_dir_recursive_absolute(ProjectSettings.globalize_path("res://artifacts/bosses"))
	var f=FileAccess.open("res://artifacts/bosses/boss-controller.json",FileAccess.WRITE)
	...
	print("BOSS_CONTROLLER_PLAY: %d failures"%failures)
