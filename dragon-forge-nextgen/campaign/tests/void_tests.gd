...
static func completed(optional_guardians: bool = false) -> Dictionary:
	...
	c.guardians.append("light")
	return c

func migration():
	var old = completed(true)
	old.version = 8
	old.guardians.erase("light")
	...
	for key in old:
		if key not in ["version", "guardians"]:check(migrated[key] == old[key], "schema 8 migration preserves " + key)
...
func recruitment():
	...
	for optional_guardians in [false, true]:
		...
		before_finish.guardians.erase("light")
		...
		check(Fusion.members(c) == expected_pair and c.active_guardian == "fire", "Void recruitment preserves the expedition pair selected before Void forging")
		check(not Rules.normalize(c).is_empty(), "new earned Void roster persists")
		check(Fusion.equip_reserve(c, "void"), "Null can replace selected reserve at Forge")
		...
		if not optional_guardians:
			...
			check(c.guardians == ["fire","light","void","ice"] and not Rules.normalize(c).is_empty(), "earned roster accepts later recruitment without exact ordering")
		else:
			c.guardians = ["fire","void","shadow","venom","stone","storm","ice","light"]
			check(not Rules.normalize(c).is_empty(), "all eight earned guardians normalize as a unique set")
