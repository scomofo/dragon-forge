...
func run():
	...
	w.hud.close_overlay()
	check(w.campaign.guardians == prior_guardians + ["void"] and Fusion.members(w.campaign) == prior_pair, "world recruitment retains all six guardians and expedition pair")
	check(not Rules.normalize(w.campaign).is_empty(), "world recovery and forging produce save-valid schema 9")
	w.dragon.position = Vector3(6.0, .1, 8.0)
	w.dragon.input_grace = 0.0
	w.hud.show_party()
	await reachable(w, "GuardianScroll", "EquipReserve_void", "Seventh guardian reserve action")
