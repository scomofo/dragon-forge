...
const ClipScript = preload("res://src/battle_content/slice_battle_animation_clip.gd")
const BindingScript = preload("res://src/battle_content/slice_battle_action_animation_binding.gd")
const ActorSetScript = preload("res://src/battle_content/slice_battle_actor_animation_set.gd")
const ManifestScript = preload("res://src/battle_content/slice_battle_animation_manifest.gd")
const MoveScript = preload("res://src/battle_content/slice_move_definition.gd")
const BattleScript = preload("res://src/battle_content/slice_battle_definition.gd")
...
func _init() -> void:
	_save(_make_manifest(), "res://assets/battle/animation_manifests/root_wyrmling_vs_admin_protocol.tres")
	...
	print("Authored prototype battle manifest content.")
	quit(0)
...
func _make_manifest() -> Resource:
	...
	manifest.notes = "Prototype-local mirror of the first validated Root Wyrmling vs Admin Protocol content fixture with dedicated attack and reaction strips."
...
func _make_root_actor_set() -> Resource:
	var actor_set: Resource = ActorSetScript.new()
...
func _make_admin_actor_set() -> Resource:
	var actor_set: Resource = ActorSetScript.new()
...
func _make_root_clips() -> Array:
	return [
		_sequence_clip(&"root_wyrmling_idle", ROOT_DIR, "root_idle", 4, 130, Vector2i(192, 144), "%s/root_wyrmling_root_idle_sheet.png" % PREVIEW_DIR, ["%s/08_battle_telegraph.png" % CAPTURE_DIR], &"approved", true),
...
func _make_admin_clips() -> Array:
	return [
		_sequence_clip(&"admin_protocol_idle", ADMIN_DIR, "admin_idle", 4, 130, Vector2i(192, 176), "%s/admin_protocol_idle_sheet.png" % PREVIEW_DIR, ["%s/08_battle_telegraph.png" % CAPTURE_DIR], &"approved", true),
...
func _sequence_clip(
	...
	var clip: Resource = ClipScript.new()
	...
	clip.accessibility_notes = "Runtime checked against vertical-slice capture."
	return clip


func _vfx_clip(clip_id: StringName, path: String, runtime_capture_path: String) -> Resource:
	var clip: Resource = ClipScript.new()
	clip.clip_id = clip_id
...
func _binding(
	...
	var binding: Resource = BindingScript.new()
...
func _make_move(
	...
	var move: Resource = MoveScript.new()
...
func _make_battle_definition() -> Resource:
	var battle: Resource = BattleScript.new()
