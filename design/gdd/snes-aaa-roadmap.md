...
The battle correctness pass shares turn resolution across normal commands and swaps, preserves party state during Siren's lure, and commits healing and boss HP changes consistently. Its controller regressions complement the pure engine tests. Save recovery has also landed. Remaining authored-boss contract checks are tracked in `TODO.md`.
