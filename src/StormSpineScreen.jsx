...
const LANE_LABELS = {
  capacitor: 'Capacitor lane — charge banked in the cells',
  archive: 'Conduit lane — the hydra logs spool below',
  direct: 'Direct bus — nothing between you and the core',
};

export default function StormSpineScreen({ save, refreshSave, onNavigate, onBeginCampaignBattle }) {
  const progress = getStormSpineProgress(save);
  const room = STORM_SPINE_ROOMS[progress.roomId];
  ...
  const [forkFx, setForkFx] = useState(null); // 'capacitor' | 'archive' | 'direct'
  ...
  const canBattle = Boolean(getStormSpineBattleConfig(save, guardian, reserve));
  const exits = getStormSpineExits(save);
  ...
  function perform(action, value, feedback = '') {
    if (!actInStormSpine(action, value)) return false;
  ...
  function travelTo(exit) {
    if (transitRef.current || forkFx) return;
  ...
  // Bespoke fork choice: the two unchosen lanes arc shut, then the choice
  // commits and its message lands.
  function chooseLane(lane, feedback) {
    if (forkFx || transitRef.current) return;
    if (reducedMotion) { perform('choose-lane', lane, feedback); return; }
    playSound('shieldDeflectSting');
    setForkFx(lane);
    later(() => {
      perform('choose-lane', lane, feedback);
      setForkFx(null);
    }, 1600);
  }

  function beginBattle() {
    const config = getStormSpineBattleConfig(loadSave(), guardian, reserve);
    if (!config) return;
    actInStormSpine('party', { guardianId: guardian, reserveId: reserve });
  ...
  if (room.inspect) {
    actions.push({
      id: 'inspect', label: room.id === 'broken-conduit' ? 'Read the origin logs' : 'Inspect the room', x: 38,
      run: () => {
        if (room.id === 'broken-conduit') perform('read-archive');
        playSound('uiConfirm');
        setMessage(room.inspect);
      },
    });
  }
  if (room.id === 'overclock-gantry' && !owned.length) actions.push({ id: 'hatch', label: 'Hatch a guardian first', x: 64, run: () => onNavigate('hatchery') });
  if (room.id === 'fork-in-the-wire' && !progress.forkLane) {
    actions.push({ id: 'lane-capacitor', label: `Capacitor lane · ${STORM_SPINE_CACHE_REWARD} scraps cached`, x: 52, run: () => chooseLane('capacitor', 'The capacitor lane holds. The other two arc shut behind you.') });
    actions.push({ id: 'lane-archive', label: 'Conduit lane · origin logs', x: 66, run: () => chooseLane('archive', 'The conduit lane opens, whispering. The other two arc shut behind you.') });
    actions.push({ id: 'lane-direct', label: 'Direct bus · nothing between', x: 80, run: () => chooseLane('direct', 'The direct bus carries you clean. The other two arc shut behind you.') });
  }
  if (room.id === 'capacitor-bank' && !progress.cacheClaimed) actions.push({ id: 'cache', label: `Drain the cells · ${STORM_SPINE_CACHE_REWARD} scraps`, x: 60, run: () => perform('claim-cache', null, `Drained ${STORM_SPINE_CACHE_REWARD} DataScraps from the bank. The cells go dark.`) });
  if (room.id === 'discharge-gate') {
    if (!progress.rewardClaimed) actions.push({ id: 'reward', label: `Collect ${STORM_SPINE_CLEAR_REWARD} scraps · one hatch`, x: 60, run: () => perform('claim-clear', null, 'Storm Spine stabilized. Your next hatch is funded. The gantry is quiet above you.') });
  ...
  for (const exit of exits) {
    if (exit.route && progress.forkLane && exit.route !== progress.forkLane) continue;
  ...
  return (
    <div className="outer-grid-screen storm-spine-screen">
      ...
      <main className="outer-grid-layout">
        ...
        <div className="outer-grid-body">
          <section className="outer-grid-exploration" aria-label="Room exploration">
            <div
              ref={sceneRef}
              className={`outer-grid-scene storm-spine-scene${transit ? ` transit-${transit.phase}-${transit.dir}` : ''}`}
              ...
              onClick={event => {
                if (event.target !== event.currentTarget || transitRef.current || forkFx) return;
                sceneRef.current.focus({ preventScroll: true });
            ...
            >
              ...
              />
              {forkFx && (
                <div className="storm-fork-fx" aria-hidden="true">
                  {['capacitor', 'archive', 'direct'].map(lane => (
                    <span key={lane} className={`fork-lane fork-lane-${lane}${lane === forkFx ? ' is-chosen' : ' is-shut'}`} />
                  ))}
                </div>
              )}
            ...
            <div className="outer-grid-actions" aria-label="Room actions">
              {actions.map(action => (
                <button key={action.id} type="button" className={activeAction === action.id ? 'is-selected' : ''}
                  disabled={action.disabled || Boolean(transit) || Boolean(forkFx)} title={action.disabled ? action.hint : undefined}
            ...
            {room.id === 'fork-in-the-wire' && !progress.forkLane && <p className="outer-grid-note">Choose one lane for this expedition. The other two arc shut — their contents stay on the wire.</p>}
            {room.id === 'capacitor-bank' && progress.cacheClaimed && <p className="outer-grid-note">The bank is drained.</p>}
          ...
          <aside className="outer-grid-party" aria-label="Guardian and encounter briefing">
            <h2>YOUR GUARDIAN</h2>
            {owned.length ? <>
              <label htmlFor="ss-guardian">Primary</label>
              <select id="ss-guardian" value={guardian} onChange={event => { setGuardian(event.target.value); if (reserve === event.target.value) setReserve(''); }}>
              ...
              <label htmlFor="ss-reserve">Reserve</label>
              <select id="ss-reserve" value={reserve} onChange={event => setReserve(event.target.value)}>
